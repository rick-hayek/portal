...
describe('API — Next.js integration', () => {
  it('tRPC route handler exists', () => {
    expect(fs.existsSync(path.join(WEB_SRC, 'app/api/trpc/[...trpc]/route.ts'))).toBe(true);
  });
  ...
  it('Route handler uses fetch adapter', () => {
    const content = fs.readFileSync(path.join(WEB_SRC, 'app/api/trpc/[...trpc]/route.ts'), 'utf-8');
...
describe('Seed data', () => {
  it('seed.ts exists', () => {
    expect(fs.existsSync(path.resolve(__dirname, '../packages/db/prisma/seed.ts'))).toBe(true);
  });
