...
describe('Blog Module — Components', () => {
  it('PostCard.tsx exists', () => {
    expect(fs.existsSync(path.join(WEB_SRC, 'components/blog/PostCard.tsx'))).toBe(true);
  });

  it('CommentSection.tsx exists', () => {
    expect(fs.existsSync(path.join(WEB_SRC, 'components/blog/CommentSection.tsx'))).toBe(true);
  });

  it('PostCard renders title, excerpt, tags', () => {
    const content = fs.readFileSync(path.join(WEB_SRC, 'components/blog/PostCard.tsx'), 'utf-8');
