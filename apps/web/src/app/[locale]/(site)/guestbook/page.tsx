...
export default function GuestbookPage() {
    ...
    return (
        <div style={{ padding: '5rem 2rem', maxWidth: 1200, margin: '0 auto', width: '100%' }}>
            ...
            {/* Entries */}
            {loading ? (
                <p className="py-8 text-center text-[var(--portal-color-text-secondary)]" style={{ fontSize: '.82rem' }}>{t('loading')}</p>
            ) : entries.length > 0 ? (
            ...
            <div className="flex border-t border-[var(--portal-color-border-soft)]" style={{ gap: '3rem', padding: '2rem 0', marginTop: '2rem' }}>
                <div className="text-center">
                    ...
                    <div className="uppercase text-[var(--portal-color-text-secondary)]" style={{ fontSize: '.72rem', fontWeight: 500, letterSpacing: '.04em', marginTop: '.2rem' }}>{t('stats.posts')}</div>
                </div>
                <div className="text-center">
                    <div className="text-[var(--portal-color-text)]" style={{ fontSize: '2rem', fontWeight: 800, letterSpacing: '-.04em' }}>12</div>
                    <div className="uppercase text-[var(--portal-color-text-secondary)]" style={{ fontSize: '.72rem', fontWeight: 500, letterSpacing: '.04em', marginTop: '.2rem' }}>{t('stats.projects')}</div>
                </div>
                <div className="text-center">
                    <div className="text-[var(--portal-color-text)]" style={{ fontSize: '2rem', fontWeight: 800, letterSpacing: '-.04em' }}>3.2K</div>
                    <div className="uppercase text-[var(--portal-color-text-secondary)]" style={{ fontSize: '.72rem', fontWeight: 500, letterSpacing: '.04em', marginTop: '.2rem' }}>{t('stats.pageViews')}</div>
                </div>
                <div className="text-center">
                    <div className="text-[var(--portal-color-text)]" style={{ fontSize: '2rem', fontWeight: 800, letterSpacing: '-.04em' }}>{total || 86}</div>
                    <div className="uppercase text-[var(--portal-color-text-secondary)]" style={{ fontSize: '.72rem', fontWeight: 500, letterSpacing: '.04em', marginTop: '.2rem' }}>{t('stats.guestbook')}</div>
                </div>
            </div>
        </div>
    );
