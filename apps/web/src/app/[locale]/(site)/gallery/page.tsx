...
export default function GalleryPage() {
  ...
  function renderWrapper({ style, ...rest }: RenderWrapperProps, context: RenderWrapperContext) {
    return (
      <div
        {...rest}
      ...
      <div className="mx-auto max-w-5xl py-12 px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">
          {Array.from({ length: 8 }).map((_, i) => (
            <div
              key={i}
              className="aspect-square animate-pulse rounded-xl bg-[var(--portal-color-surface)]"
            />
          ))}
        </div>
      </div>
