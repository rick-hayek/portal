  ...
  return (
    <div className="mx-auto max-w-4xl py-12 px-4 sm:px-6 lg:px-8 xl:px-0">
      ...
      <div className="hidden print:block mb-8 text-center pb-8 border-b border-[var(--portal-color-border)]">
        <h1 className="text-4xl font-black text-[var(--portal-color-text)] tracking-tight">
          {resume.name}
        </h1>
        <p className="mt-2 text-xl text-[var(--portal-color-primary)] font-medium">
          {resume.title}
        </p>
      ...
      <div className="grid grid-cols-1 gap-12 md:grid-cols-[1fr_2.5fr]">
        ...
        <div className="space-y-8">
          ...
          <section className="print:hidden space-y-4 rounded-2xl bg-[var(--portal-color-surface)] p-6 border border-[var(--portal-color-border)] shadow-sm">
            <h2 className="text-xs font-bold uppercase tracking-widest text-[var(--portal-color-text-secondary)]">
              {t('contact')}
            </h2>
              ...
              {resume.email && (
                <li>
                  <a
                    ...
                    className="hover:text-[var(--portal-color-primary)] transition-colors"
                  >
                    {resume.email}
                  </a>
                </li>
          ...
          {/* Skills */}
          <section>
            <h2 className="mb-4 text-xs font-bold uppercase tracking-widest text-[var(--portal-color-text-secondary)]">
              {t('skills')}
            </h2>
            <div className="space-y-6">
              {resume.skills.map((category, idx) => (
                <div key={idx}>
                  <h3 className="mb-2 text-sm font-semibold text-[var(--portal-color-text)]">
                    {category.title}
                  </h3>
        ...
        <div className="space-y-12">
          ...
          {/* Experience */}
          <section>
            <h2 className="mb-8 text-2xl font-bold text-[var(--portal-color-text)]">
              {t('experience')}
            </h2>
            <Timeline
