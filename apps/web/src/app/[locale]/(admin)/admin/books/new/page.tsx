...
export default function AdminNewBookPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const res = await fetch('/api/trpc/admin.bookCreate?batch=1', {
        method: 'POST',
      ...
      const data = await res.json();
      if (data[0]?.error) {
        setError(data[0].error.message ?? 'Failed to add book');
      } else {
        startTransition(() => {
      ...
      <div className="flex items-center justify-between border-b border-[var(--portal-color-border)] pb-4">
        <div>
          ...
          <p className="text-sm text-[var(--portal-color-text-secondary)]">
            Fill in the details to add a recommended book to your library.
          </p>
        </div>
        <Link
          href="/admin/books"
