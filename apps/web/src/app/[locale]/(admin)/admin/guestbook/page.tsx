      ...
      <div className="space-y-3">
        {loading ? (
          ...
          entries.map((e) => (
            <div
              key={e.id}
              className="flex items-start justify-between rounded-xl border border-[var(--portal-color-border)] bg-[var(--portal-color-surface)] p-4"
            >
              <div>
                <span className="text-sm font-medium text-[var(--portal-color-text)]">
                  {e.authorName}
                </span>
                <p className="mt-0.5 text-sm text-[var(--portal-color-text-secondary)]">
                  {e.content}
                </p>
