...
function StatCard({ label, value, accent }: { label: string; value: number; accent?: boolean }) {
  return (
    <div className="rounded-xl border border-[var(--portal-color-border)] bg-[var(--portal-color-surface)] p-5">
      ...
      <p
        className={`mt-1 text-3xl font-bold ${accent ? 'text-[var(--portal-color-primary)]' : 'text-[var(--portal-color-text)]'}`}
      >
    ...
    async function load() {
      try {
        const [statsRes, commentsRes, guestbookRes] = await Promise.all([
          fetch(
            '/api/trpc/admin.stats?batch=1&input=' +
              encodeURIComponent(JSON.stringify({ '0': { json: null } })),
          ),
          fetch(
            '/api/trpc/admin.recentComments?batch=1&input=' +
              encodeURIComponent(JSON.stringify({ '0': { json: {} } })),
          ),
          fetch(
            '/api/trpc/admin.recentGuestbook?batch=1&input=' +
              encodeURIComponent(JSON.stringify({ '0': { json: {} } })),
          ),
        ]);
          ...
          {comments.length === 0 ? (
            ...
            <ul className="space-y-3">
              {comments.map((c) => (
                <li
                  key={c.id}
                  className="border-b border-[var(--portal-color-border)] pb-2 last:border-0"
                >
