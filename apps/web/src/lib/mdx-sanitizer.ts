...
const STANDARD_HTML_TAGS = new Set([
  ...
  'track', 'u', 'ul', 'var', 'video', 'wbr', 'svg', 'path', 'g', 'circle', 'rect',
  'line', 'polyline', 'polygon', 'text', 'use', 'defs', 'symbol'
]);
  ...
  const sanitizedTokens = tokens.map((token) => {
    ...
    text = text.replace(/<([^>\n]*)(>|$)/g, (fullMatch, tagInner, closingBracket) => {
      ...
      if (STANDARD_HTML_TAGS.has(lowerTagName)) {
        const isSelfClosing = trimmedInner.endsWith('/') || ['br', 'hr', 'img', 'input', 'meta', 'link'].includes(lowerTagName);
        if (isSelfClosing) {
          return fullMatch;
        }
