...
3. **External Memory & Sandboxed Persistence**:
   - Write each domain report (`code-structure.md`, `frontend-structure.md`, etc.) to the target output directory (e.g. `analysis-report/`) **immediately** upon phase completion.
...
5. **Tool Call & Workspace File Writing Rule**:
   - When writing report files to the user's workspace directory (e.g. `analysis-report/` or a user-specified path), use standard `write_to_file` **WITHOUT** providing `ArtifactMetadata`.
...
6. **Two-Pass Executive Synthesis**:
   - Synthesize the final `PROJECT_ANALYSIS_REPORT.md` exclusively from the persisted `analysis-report/*.md` files, without re-reading raw source code.
...
Initialize target directory (e.g. `analysis-report/` or user-specified folder). For each sub-step:
...
2. **Synthesize Reports**:
   - Read the 5 generated markdown documents in `analysis-report/`.
